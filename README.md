...
`scripts/items.py` is the authoring source for the `vibecheck_v1` framework view: the 89-item bank in four wordings, the verification metadata, and `SCANNER_CHECKS` (which checklist items each scanner check covers, and at which tier). It feeds the generator only. The canonical runtime contract is the generated registry and framework mapping (`scripts/gen_canonical.py`); the workbook, checklist map and assessment adapters consume those, and `tests/test_coverage_map.py` and `tests/test_framework_mappings.py` fail if the scanner, the item bank, the generated map, or the counts quoted in this README drift apart. Direct runtime reads of the positional tuples are deprecated through schema 1.10.0 (§11.4 of RFC 0001).
...
Selection is deterministic — a live observation outranks a source reading, a decisive method outranks an indicative one, and the declared fallback order breaks the rest, ending in a provider ID so the same inputs always produce the same plan. The chain for authorization is the Supabase two-account probe, then a Playwright two-account flow, then a guided browser test, then code and policy review. What matters more than the ranking is that nothing disappears and nothing is overstated: a stronger method that was refused for a missing credential, an absent install, an unauthorized network call or an unauthorized write is reported as a coverage gap naming the exact grant that would have enabled it, while a method with nothing to observe here is reported as inapplicable rather than as work somebody forgot. Effects that need authorization turn a plan step into a request rather than an instruction, checked per cell as well as per provider: a read-only grant runs the probe's read and reports its insert as a gap. Authorization is also scoped to one environment — permission to probe the pilot is not permission to probe production, and a live method offered a mismatched scope is refused rather than quietly re-pointed. Covering every requested cell establishes the requirement, never the control — closure stays with the coverage model and the assessment rules. Rule R24 holds provider evidence to the capability it named, checked per claimed control rather than pooled across them, so a source reading is structurally unable to carry a coverage cell and one covered control cannot vouch for another. `vibecheck.sh`, `analyze_sql.py` and `supabase_probe.py` are registry providers now; their CLI output is unchanged, the first two answer `--capability` with their own record, and the adapters attach the capability *as exercised* to every envelope they build. Thirteen worked plans are pinned in `tests/golden/providers/selection.md`.

Increment 7 puts the maintained specialist tools behind the same contract (schema version 1.6.0, additive). Gitleaks, TruffleHog, OSV-Scanner, Trivy, Semgrep CE, CodeQL, OWASP ZAP and the Playwright two-account flow are registry providers with declared coverage, cost, egress, side effects, authorization requirements, the exact command each result came from, and `run_semantics` — what a timeout, a cancellation, a crash and a partial result mean for the coverage that run claimed. `scripts/external_adapters.py` imports what each tool produced; it never runs one, installs one, or uploads anything. The scan skill is the runner for a vibecoder review: one consent, then the default pack (Gitleaks, Semgrep, OSV-Scanner); live tools still need a separate authorized ask. The Python remains import-only. Four properties hold structurally rather than by convention. A tool that is not on `PATH` produces an envelope with no evidence, one open to-do naming the controls nobody looked at, and the capability record saying what it would have covered. Every claim is intersected with the provider's declared coverage, so a rule that maps to nothing that provider declares becomes a triage to-do naming the rule instead of a claim filed under a control it may not belong to. ZAP and Playwright imports require the target URL and who authorized the run as arguments, and raise without them. Secret-bearing fields are dropped before anything enters the envelope and the rest goes through the scanner's own redaction. Only Playwright fills authorization coverage cells, only when the assertion names the object it touched, and a timed-out assertion records `inconclusive` — never `denied`, because a test that never got an answer did not watch anything be refused.

Increment 8 makes the normalized model canonical (schema version 1.6.0, additive, issue #10). The workbook, checklist map and legacy adapters now consume the `vibecheck_v1` framework mapping (`schema/mappings/vibecheck_v1.json`) instead of reading `items.py` tuples at runtime, with byte-identical output — cutover, not a behaviour change. The mapping and each entry carry explicit provenance (source, review, change policy, per-edge rationale), and item <-> control edges are many-to-many: a framework item can span several controls via `related_control_ids`, and a control can satisfy many items across frameworks. A second sample framework, `founder_focus` (`schema/mappings/founder_focus_v1.json`), reuses the same control records without duplicating any of them, which is how future frameworks are meant to reuse controls and evidence. Legacy workbook rows migrate through `adapters.import_workbook_rows` into canonical assessments — status, notes, evidence references, acceptance and item mapping all round-trip exactly — and historical envelopes keep the schema, registry and mapping versions that were current at assessment time. A row the workbook's own gates forbid (Critical marked Accepted, an acceptance or a Pass with no reason, a screening status on an ordinary control) is refused and reported rather than imported, so the importer never reports success on an envelope the validator would reject. Direct legacy paths have a documented deprecation window through schema 1.10.0 with no silent behaviour change.
