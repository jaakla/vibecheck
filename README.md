...
- Choosing a verification method never authorizes running it. A selected plan that reaches the network, uses a credential, writes, deploys, or acts in an external account is a request naming the provider, the effects and the destinations; the run starts after that exact request is granted, not before.
